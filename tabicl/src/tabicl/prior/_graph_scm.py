...
class GraphSCM:
    ...
    num_classes : int or list[int], default=2
        Number of classes per categorical target. A list generates jointly sampled
        targets and returns y shaped (seq_len, n_targets); an int keeps (seq_len,).
    ...
    **kwargs : dict
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(num_classes, list) and (regression or not num_classes or any(k < 2 for k in num_classes)):
            raise ValueError("Multiple targets require a nonempty list of class counts >= 2 and classification")
    ...
    def __call__(self) -> None:
        ...
        properties = DatasetProperties(
            ...
            cat_sizes={
                ...
                "y": [0] if self.regression else (self.num_classes if isinstance(self.num_classes, list) else [self.num_classes]),
            },
        )
        ...
        if self.regression:
            ...
            sizes = self.num_classes if isinstance(self.num_classes, list) else [self.num_classes]
            y = data["y_cat"].reshape(self.seq_len, len(sizes))
            if self.permute_labels:
                y = torch.stack([torch.randperm(k, device=self.device)[y[:, i].long()]
                                 for i, k in enumerate(sizes)], dim=-1)
            if not isinstance(self.num_classes, list):
                y = y[:, 0]  # Preserve the single-target API.
